...
export async function createTitleEmbed(title: string, description: string, colour: ColorResolvable, author: EmbedAuthorData | User): Promise<MessageEmbed> {
...
export async function createThumbnailEmbed(description: string, colour: ColorResolvable, thumbnail: string): Promise<MessageEmbed> {
    const embed = new MessageEmbed()
        .setDescription(description)
        .setColor(colour)
...
export async function createErrorEmbed(description: string, author: EmbedAuthorData | User): Promise<MessageEmbed> {
    const _author: EmbedAuthorData = author instanceof User ? { name: author.username, iconURL: author.avatarURL()! } : author
    const embed = new MessageEmbed()
        .setDescription(description)
        .setColor('#ff0000')
